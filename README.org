...
** collision mechanic realism
Spawn fewer npc ships before testing this next thing.

You can detect the "head-on-edness" of collisions. Such collisions should retard your speed. This will prevent pinballing in cluttered areas.

Rear-on collisions should boost speed so that asteroids that come from behind on a stationary ship (especailly in the asteroids scenario) don't rapidly obliterate a ship.

Have a sufficiently head on collision (measure the angle of collision) cut speed in half.

Have a sufficiently rear-on collision add the other collider's speed to self.

Are npc ships giving capital ships a wide berth?
** have pick-ups explode out of bad guys
When bad guys die, give a chance of exploding out a pickup object.
pickup should be a new object instance thing and it should use the question mark image. It should have properties like a gem.
Pickup contains one of the things the dead enemy had: weapon, missile, mine, engine, or ship.
Upon collision with the player, pickup puts the item in the player's hold and displays the text "New ___!" where the pickup was grabbed where ____ is filled in by the type of the pickup.

Make stuff in your cargo hold such as engines and weapons take up cargo space.
When player runs over a pickup, but cargo is full, display "Cargo full!" instead of New ___! and do not put the item in the cargo hold.

Add option to salvage stuff in your cargo hold for money from the comparator view.
Money gained is based on the class of the object. Some low-level items should give no money. You don't have to get much money from these since cargospace is valuable.
** better interaction with factions
at any faction-owned node, pop up a menu
add to the menu options to do any of the minigames. lay out with text and numbers exactly the before and after consequences of each minigame whether won or lost and the difficulty of the minigame.
finally make each minigame link playable.
next make winning and losing actually have an effect, as follows: winning has the following effects, losing has only the negative consequences of the following:

asteroids - decrease asteroids at this node, make faction like you
gemwild - decrease wealth at this node, but you get to keep wealth you pick up, make faction dislike you
race - increase faction tech at this node, make faction like you
furball - decrease faction tech at this node, make faction dislike you, increase asteroids at this node
capital ship battle - decrease faction production at this node, make faction dislike you, increase asteroids at this node

Add node attributes production and an array of counts of ships of each class that are present. Actually, production will also need to keep track of the classes being produced at the node.
...
Factions store the following attributes:
  tech level (stored at the node level)
  money
  amount of each resource (don't do this yet)
  nodes owned
  for each class level, number of ships owned at that class (at each node)
  number of capital ships (at each node)
  relationship with player
  relationship with other factions (do this later)
  shipyards and their quality at each node

Disable menu use except for pausing and the help menu when playing minigames. Otherwise the player can flee the minigame back to infinite space.

How to handle player death during minigames? Just reset everything for now. Make sure this happens.

Have the minigame not finish immediately upon victory. Have it wait. For example, we want to watch the capital ship explode. Put in a little delay.

You should write a text menu to introduce each minigame as well, instead of text flashing on the screen on a black background.
That is, replace the announcements in scenarios.py with a little text menu intro.

Modify faction options:
...
Make ships affiliated with a faction or with None.
...
But also make this anger the faction and sour your relationship with them.
...
** random health bars floating in space
Push this off until you notice any floating in space.
Find where one of these appears consistently (random num should be used but used with initial seed).
Then take dramatic measures to figure out when and why it appears.
For example, if you spawn no enemies, does the health bar still appear?
...
** make different levels of enemies
And deploy them based on node strength or whatever.

The different npcs can have different levels relatively easily based on the different classes of items I've implemented.

Make different levels of capital ships too!

Scale this for the pirate ships with distance from the player's starting node. Make it a relatively shallow scaling with a cap. Only factions should be able to get higher tech than this cap.
...
https://github.com/nealholt/asteroids-buff
