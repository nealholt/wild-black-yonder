...
TODO LEFT OFF HERE in scenarios.epicBattle.

capital ship needs fixed to work with teams. it can no longer use globalvars.player_target_lead as this no longer exists.

1. just add some ships on opposite sides of space to the two teams.
2. change the ships to give points depending on their team.
3. make sure the points work out so the winning is correct.
4. implement capital ships and add them to the battle.
