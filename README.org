...
Add node attributes production and tech, and an array of counts of ships of each class that are present. Actually, production will also need to keep track of the classes being produced at the node.

Make it so the following attributes are either stored at the faction or the node as needed:
