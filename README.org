...
cgp - new parallel folder. different ways of adding to novelty archive.

cgp - How else can I debug? There must be a better way

pygame. Finish off minimap next step. Just a little step. Then clearly mark where you left off.



** minimap
Left off in menus.py

Display player, warp point, fuel depots, and planet on the minimap.
** Enable setting of destination to any of the objects in the minimap
** new NPC behaviors
