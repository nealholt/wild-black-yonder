...
def run(countdown=-1):
	...
	global dirty_rects, take_screenshot
	...
	while running:
		...
		if update_mechanism == DIRTY:
			...
			above_threshold = (len(dirty_rects) > UPDATE_THRESHOLD)
			if above_threshold:
				pygame.display.flip()
			else:
				pygame.display.update(dirty_rects)
		...
		#TODO LEFT OFF HERE - TESTING - this can be improved. not all of these are on screen, but we know which ones are!!!
		if update_mechanism != FLIP:
			#This is where we cover (ie, draw the background over) rects we drew
			#on the screen in the previous update.
		...
		if update_mechanism == DIRTY or (update_mechanism == DYNAMIC and not above_threshold):
