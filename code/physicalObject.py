	...
	def collisionAvoidance(self):
		...
		if self.closest_sprite is None:
			...
			if abs_angle < self.danger_cone and self.dist_to_closest < self.suppress_turn_threshold:
				#Angle to target, size of target, self acceleration,
				#and turn rate decay
				#should all factor into a decision here.
				#Figure out optimal speed and amount to turn
				fraction = min(1.0, 
					(self.dv*self.dist_to_closest) / (self.closest_sprite.collisionradius*abs_angle*self.turnRateDecay*self.speed_safety_factor)
					)
