...
def getClosestLeadIndicator(location, ship_list):
	'''Returns the nearest ship in ship_list or None.'''
	#If there are no ships then return None
	if len(ship_list) == 0: return None
	#Initialize min_dist and nearest from the 0th ship in the list.
	nearest = ship_list[0]
	min_dist = cygeometry.distance(location, ship_list[0].lead_indicator)
	#Calculate the nearest ship to the location
	for i in range(1, len(ship_list)):
		d = cygeometry.distance(location, ship_list[i].lead_indicator)
		if d < min_dist:
			min_dist = d
			nearest = ship_list[i]
	return nearest
...
class CapitalShip(physicalObject.PhysicalObject):
	...
	def setTarget(self):
		if self.team == globalvars.REDTEAM:
			#Pick blue team targets
			for i in range(len(self.gunlocs)):
				self.targets[i] = getClosestLeadIndicator(self.gunlocs[i],\
							globalvars.BLUE_TEAM.sprites())
		else:
			#Pick red team targets
			for i in range(len(self.gunlocs)):
				self.targets[i] = getClosestLeadIndicator(self.gunlocs[i],\
							globalvars.RED_TEAM.sprites())
