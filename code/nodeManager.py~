...
class Node():
	...
	def refreshObstacles(self, seed=0):
		'''Get a list of obstacles based on the attributes of this node.'''
		rd.seed(seed) #Fix the seed for the random number generator.
		self.obstacles['enemy'] = int(rd.uniform(self.enemy_min, self.enemy_max))
		self.obstacles['crystal'] = int(rd.uniform(self.crystal_min, self.crystal_max))
		self.obstacles['large_asteroid'] = int(rd.uniform(self.large_asteroid_min, self.large_asteroid_max))
		self.obstacles['medium_asteroid'] = int(rd.uniform(self.medium_asteroid_min, self.medium_asteroid_max))
		self.obstacles['small_asteroid'] = int(rd.uniform(self.small_asteroid_min, self.small_asteroid_max))
		self.obstacles['gold_metal'] = int(rd.uniform(self.gold_metal_min, self.gold_metal_max))
		self.obstacles['silver_metal'] = int(rd.uniform(self.silver_metal_min, self.silver_metal_max))
		self.obstacles['health'] = int(rd.uniform(self.health_min, self.health_max))
		self.obstacles['capital_ship'] = int(rd.uniform(self.capital_ship_min, self.capital_ship_max))
