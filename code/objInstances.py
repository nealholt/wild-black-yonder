...
class Explosion(PhysicalObject):
	...
	delay is the frame delay between subsequent explosions. This is meant to reduce the burden of displaying a lot of explosions.'''
	def __init__(self, x=0, y=0, xMinAdj=0, xMaxAdj=0, yMinAdj=0, yMaxAdj=0, ttl=0.5, delay=0):
		PhysicalObject.__init__(self, centerx=x, centery=y)
		self.delay = delay
		self.delay_count = 0
...
class Asteroid(PhysicalObject):
	...
	def handleCollisionWith(self, other_sprite):
		...
		if other_sprite.is_a == globalvars.BULLET:
			...
			#Spawn random debris from the point of impact.
			amount_of_debris = rd.randint(0,4)
			for _ in range(amount_of_debris):
...
class FinishBullsEye(PhysicalObject):
	...
	def __init__(self, racer, target):
		PhysicalObject.__init__(self, \
			centerx=target[0], \
			centery=target[1],\
	...
	def update(self):
		''' '''
		#Distance to target
		self.dtt = cygeometry.distance(self.racer.rect.center, self.target)
	...
	def draw(self, offset):
		...
		#Check if the racer has reached the destination.
		if self.dtt < 40:
			#If so, end the race.
