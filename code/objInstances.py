...
class Flash(physicalObject.PhysicalObject):
	...
	def __init__(self, x=0, y=0, flashCenterRadius = 20, flashRadiusMin = 20, flashRadiusMax = 50, flashMinTimeToLive = 8, flashMaxTimeToLive = 17):
...
class Debris(physicalObject.PhysicalObject):
	def __init__(self, x=0, y=0, minTTL=10, maxTTL=24, minSpeed=7.0, maxSpeed=30.0, minTheta=-179, maxTheta=180):
		physicalObject.PhysicalObject.__init__(self, \
			centerx=x, centery=y, width=4, height=4)
		self.timeToLive = rd.randint(minTTL, maxTTL)
		self.theta = rd.randint(minTheta, maxTheta)
		self.speed = rd.randint(minSpeed, maxSpeed)
