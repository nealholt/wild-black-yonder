...
def getLocalGalaxyPanel(travel): #TODO
	'''Pre: galaxy is a NodeManager object that has been initialized.'''
	menu = getStandardMenu()
	radius = 10
	padding = 20
	...
	playerNodeLoc = (globalvars.CENTERX-radius, globalvars.CENTERY-radius)
	...
	subpanel.setMethod(scenarios.setDestinationNode)
	temp = drawable.Circle(x1=playerNodeLoc[0], y1=playerNodeLoc[1],
				radius=radius, color=colors.red)
	subpanel.addDrawable(temp)
	menu.addPanel(subpanel)

	#Connections
	for c in playerNode.connections: #id,location pairs
		angle = angleFromPosition(playerNode.loc, c[1])
		dist = distance(playerNode.loc, c[1])
		position = translate(playerNodeLoc, angle, dist*magnitude)
		#Draw node if it is on screen
		...
		position[1] < globalvars.HEIGHT-padding-border_padding:
			subpanel = Panel()
			subpanel.argument = c[0]
			subpanel.setMethod(scenarios.setDestinationNode)
			temp = drawable.Circle(x1=position[0]-radius, y1=position[1]-radius,
						radius=radius, color=colors.yellow)
			subpanel.addDrawable(temp)
			menu.addPanel(subpanel)
		#connect it to the playerNode
		temp = drawable.Line(x1=playerNodeLoc[0], y1=playerNodeLoc[1],
					x2=position[0]-radius, y2=position[1]-radius)
		menu.addDrawable(temp)
	return menu
