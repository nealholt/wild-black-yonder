...
class Menu:
	...
	def setMinimapPanel(self):
		...
		#get all the locations you want to plot.
		#Draw a circle like a radar.
		radius = height/2-font_size
		center = (left+width/2, top+height/2)
		subpanel = Panel()
		temp = drawable.Circle(x1=center[0], y1=center[1], \
			radius=radius, color=colors.blue)
		subpanel.addDrawable(temp)
		self.main_panel.addPanel(subpanel)
		#Plot all the other objects initially as letters.
		to_plot = []
		#Plot warps
		for x in globalvars.galaxy.player_node.warps:
			angle = angleFromPosition(globalvars.player.rect.center, x.rect.center)
			dist = cygeometry.distance(globalvars.player.rect.center, x.rect.center)
			to_plot.append(['W',angle,dist, x])
		#Plot planets
		for x in globalvars.galaxy.player_node.planets:
			angle = angleFromPosition(globalvars.player.rect.center, x.rect.center)
			dist = cygeometry.distance(globalvars.player.rect.center, x.rect.center)
			to_plot.append(['P',angle,dist, x])
		#Plot fuel depots
		for x in globalvars.galaxy.player_node.fuel_depots:
			angle = angleFromPosition(globalvars.player.rect.center, x.rect.center)
			dist = cygeometry.distance(globalvars.player.rect.center, x.rect.center)
			to_plot.append(['F',angle,dist, x])
		#Find the largest distance and then set the scaling so that the distance is at
		#the edge of the radar and everything else is nearer.
		largest_dist = 0.0
		for x in to_plot:
			if x[2] > largest_dist:
				largest_dist = x[2]
		#Mark the player's location with a little red dot
		subpanel = Panel()
		temp = drawable.Circle(x1=center[0], y1=center[1], \
			radius=5, color=colors.red)
		subpanel.addDrawable(temp)
		self.main_panel.addPanel(subpanel)
		#Draw all the objects.
		for x in to_plot:
			position = translate(center, x[1], (x[2]/largest_dist)*radius)

			subpanel = Panel()
			temp = drawable.Circle(x1=position[0], y1=position[1], \
				radius=10, color=colors.yellow)
			subpanel.addDrawable(temp)
			subpanel.setMethod(globalvars.scenario_manager.setArrowTarget)
			subpanel.argument = x[3]
			self.main_panel.addPanel(subpanel)

			subpanel = Panel()
			temp = drawable.Text(x1=position[0]-5,\
				y1=position[1], string=x[0],\
				font_size=font_size, color=colors.black)
			subpanel.addDrawable(temp)
			subpanel.setMethod(globalvars.scenario_manager.setArrowTarget)
			subpanel.argument = x[3]
			self.main_panel.addPanel(subpanel)
		#Print the legend with how far things are on the bottom left of the screen.
		self.addTextToMainPanel(['1 radius = '+str(int(largest_dist))+' pixels'], left, height-font_size)
	...
	def setGalaxyPanel(self, travel, text=None):
		...
		if len(globalvars.player.destinationNode) > 0:
			start = globalvars.galaxy.player_node
	...
	def setLocalGalaxyPanel(self, travel):
		...
		for n in globalvars.galaxy.nodes:
			angle = angleFromPosition(globalvars.galaxy.player_node.loc, n.loc)
			dist = cygeometry.distance(globalvars.galaxy.player_node.loc, n.loc)
			...
			n.id != globalvars.galaxy.player_node.id:
				#Draw it
				subpanel = Panel()
				subpanel.argument = n.id
	...
	def setRestartPanel(self):
		#First find if there is a timeLimitDisplay in intangibles. 
		#If so, then the player should also lose the mission he was in.
		mission_text = ''
		for i in globalvars.intangibles_top:
			if i.is_a == globalvars.TIMELIMITDISPLAY:
				mission_text = 'You lost the mission '+i.mission.name
				i.loseMission(suppress_menu=True)
				break
		globalvars.disable_menu = False
		self.setStandardMenu(tabs=False)
		#Then draw the contents of the menu
		#Display text explaining that player died.
		self.addTextToMainPanel(['You have died.', mission_text], globalvars.WIDTH/2-100, 200)
